...
# from reword_model import RewardModel, compute_rank_list_loss
...
def train(reward_model, loss_fn, writer, args):
    ...
    for epoch in range(1, args.num_train_epochs + 1):
        for batch in train_dataloader:
            ...
            loss = loss_fn(batch_rank_rewards, device=args.device)
            loss.backward()
            optimizer.step()
            lr_scheduler.step()
            optimizer.zero_grad()
            ...
            if global_step % args.valid_steps == 0:
                ...
                acc = evaluate_model(model, eval_dataloader, args)
